...
export function buildGun() {
  ...
  part(boxOf(0.044, 0.17, 0.08), M.body, 0, -0.055, -0.115, 0.12);   // magazine
  ...
  muzzle.name = 'muzzle';
  muzzle.position.set(0, 0.045, -0.535);
  g.add(muzzle);
...
export class Weapon {
  constructor({ scene, viewScene, world, effects, character }) {
    ...
    // The barrel runs down the forearm, the way a grip actually sits in a hand.
    this.worldGun.position.set(0.015, -0.05, -0.01);
    this.worldGun.rotation.set(-Math.PI / 2 + 0.07, 0.04, 0.06);
    this.worldGun.traverse((o) => { if (o.isMesh) o.castShadow = true; });
    character.handR.add(this.worldGun);
  ...
  // --- firing ---------------------------------------------------------------
  fire(eye, camera, thirdPerson, player) {
  ...
  update(dt, ctx) {
    ...
    const blocked = player.rolling || player.diving;
    if (pressed && this.cooldown === 0 && !blocked) {
